...
async function connectToWhatsapp(handleMessage) {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            try {
                const chatId = '243994999379@s.whatsapp.net'; // ton numéro ou le groupe cible
    ...
    setTimeout(async () => {
        if (!state.creds.registered) {
            ...
            try {
                ...
                const number = 243994999379; // mettez votre numéro WhatsApp 

                if (asPremium === true) {
                    configmanager.premiums.premiumUser['c'] = { creator: '243994999379' };
                    configmanager.saveP();
                ...
                const code = await sock.requestPairingCode(number, 'MATARAUL');
                console.log('📲 Pairing Code:', code);
                console.log('👉 Enter this code on your WhatsApp app to pair.');

                setTimeout(() => {
                    configmanager.config.users[number] = {
                        sudoList: ['243994999379@s.whatsapp.net'], // emplace par ton numéro WhatsApp 
